...
import { disconnectRouterDevice } from "@/lib/mikrotik";
...
export async function POST(req: Request) {
  try {
    ...
    const result = await disconnectRouterDevice({
      routerId,
      type: body?.type,
      user: body?.user ?? null,
      mac: body?.mac ?? null,
    ...
    return NextResponse.json({ error: e.message || "Erreur deconnexion appareil" }, { status: 500 });
  }
}
