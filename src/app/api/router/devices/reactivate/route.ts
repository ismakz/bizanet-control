...
import { reactivateRouterDeviceUser } from "@/lib/mikrotik";
...
export async function POST(req: Request) {
  try {
    ...
    const result = await reactivateRouterDeviceUser({
      routerId,
      type: body?.type,
      user: body?.user ?? null,
      mac: body?.mac ?? null,
    ...
    return NextResponse.json({ error: e.message || "Erreur reactivation appareil" }, { status: 500 });
  }
}
